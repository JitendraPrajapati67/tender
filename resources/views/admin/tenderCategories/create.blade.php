...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tenderCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tender-categories.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="category_code">{{ trans('cruds.tenderCategory.fields.category_code') }}</label>
                <input class="form-control {{ $errors->has('category_code') ? 'is-invalid' : '' }}" type="text" name="category_code" id="category_code" value="{{ old('category_code', '') }}" required>
            ...
            <div class="form-group">
                <label class="">{{ trans('cruds.tenderCategory.fields.parent_id') }}</label>
                <select class="form-control {{ $errors->has('parent_id') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id" >
                    ...
                    @foreach($parentCategory as $key => $label)
                        <option value="{{ $label->id }}" {{ old('parent_id', '') === (string) $label->id ? 'selected' : '' }}>{{ $label->category_name }}</option>
                    @endforeach
                </select>
