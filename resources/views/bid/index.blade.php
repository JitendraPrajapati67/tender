            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-TenderCategory">
                <thead>
                    <tr>
                        <th width="10"></th>
                        <th>Tender Title</th>
                        <th>{{ trans('cruds.bidManagement.fields.price') }}</th>
                        <th>{{ trans('cruds.bidManagement.fields.description') }}</th>
                        <th>{{ trans('cruds.bidManagement.fields.created_at') }}</th>
                        <th>{{ trans('cruds.bidManagement.fields.updated_at') }}</th>
                ...
                <tbody>
                    @foreach($bids as $key => $bid)
                        <tr data-entry-id="{{ $bid->id }}">
                            ...
                            <td>{{ $bid->price ?? '-' }}</td>
                            <td>{{ $bid->discription ?? '' }}</td>
                            <td>{{ date('Y-m-d', strtotime($bid->created_at)) ?? '' }}</td>
                            <td>{{ date('Y-m-d', strtotime($bid->updated_at)) ?? '' }}</td>
                            <td>
