...
class MaterialController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('parent', function ($row) {
                return $row->parent ? Material::where('id',$row->parent)->first()->category_name : '-';
            });
            $table->editColumn('category_name', function ($row) {
                return $row->category_name ? $row->category_name : "";
            });

            $table->rawColumns(['actions', 'placeholder'])->addIndexColumn();
    ...
    public function create()
    {
        abort_if(Gate::denies('material_create'), Response::HTTP_FORBIDDEN, '403 Forbidden');
         $parentCategory = Material::where('parent',null)->get();
        return view('admin.materials.create',compact('parentCategory'));
    }
